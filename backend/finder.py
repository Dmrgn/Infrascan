...
def analyze(address, secret, email):
    ...
    geocode = address_to_formatted_geocode(address, True)
...
def address_to_formatted_geocode(address, include_region_info=False):
    ...
    if include_region_info:
        if len(geocode["context"]) >= 3:
            data["region"] = f'{geocode["context"][-3]["text"]}, {geocode["context"][-2]["text"]}, {geocode["context"][-1]["text"]}'
        else:
            data["region"] = f'{geocode["context"][-1]["text"]}'
    return data

# creates a map url given the geocode of the analyzed location
# and the analysis from the analyze function
...
def analyze_location(geocode):
    ...
    for term in SEARCH_TERMS:
        ...
        total_score += results[-1]["term_score"]
    return {
        "results": results,
        "score": min((total_score/7)*20, 100) # make score between 0-100 (kinda abitrary values after experimentation)
    }
